...
//para criar uma nova instrução, faça um que seja parecido com algum que já existe, como ter o mesmo número de argumentos
//tem que mudar em 3 lugares diferentes: codigo, montador e manual (só para dizer o que faz e qual a sintaxe)
/*
Perguntas:
...
#define MIN 38 // INSTRUÇÃO NOVA AQUIIIIIII "100110" ---> Binário
	...
	switch(state)
	{
		...
		case STATE_DECODE:
			...
			switch(opcode){
				...
				case MOV:
					//o reg mais perto do nome da instrução recebe 
					//mov - move de um registrador para o outro (dificilmente é usado)
					selM4 = ry;
					selM2 = sM4;
					LoadReg[rx] = 1;
				...
				case LNOT:
					...
					selM2 = sULA;
					OP = opcode;
					//o pega_pedaco se refere à quais valores serão pegos dos bits dos 16 (0 - 15) 
				...
				case MIN: //INSTRUÇÃO NOVA AQUIIIIIIIIIIIIIIIIIII
					selM3 = ry;
					selM4 = rz;
					selM2 = sULA;
					OP = opcode;
				...
				case CMP:   // seta 3 flags: maior, menor ou igual
					//if(rx == ry)
					//no compara, o mais perto do nome da instrução n recebe o valor, vai ser o FR (flag register) q vai receber
					selM3 = rx;
					selM4 = ry;
					carry = 0;
					OP = CMP;
				...
				//o call chama uma função, assim como o Jump
				case CALL:
					COND = pega_pedaco(IR,9,6);
					...
					if((COND == 0)                       	                      // NO COND
							...
							|| (FR[9]==1 && (COND==13)))                          // DIVBYZERO
					{ // PC = MEMORY[PC];
						//tem que copiar o PC para a pilha
						//SP(Stack Pointer) - sempre aponta para a caixinha vazia no topo da pilha
						selM1 = sSP; //deixa o MUX 1 passar o SP
						RW = 1; //coloca para o modo de escrita
						selM5 = sPC; //deixa o MUX 5 passar o PC
						DecSP = 1; //decrementa para subir na memória, ir para o topo da pilha (o próximo vazio)
						state=STATE_EXECUTE;
					}
					else{
						//PC++;
